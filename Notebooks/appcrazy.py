...
fliepath = 'vehicles_us.csv'
df_vhs = pd.read_csv(fliepath)
...
def pre_process(vhs):
    ...
    vhs['Manufacturer'] = df_vhs['Model'].str.split().str[0]
...
if not small_manus:
    ...
    large_manus = manu_counts[manu_counts >= 1000].index
...
cond_hist = px.histogram(df_user_years[df_user_years['Condition'].isin(selected_condition)], x='Model Year',
                            color='Condition', title='Distribution of Model Year by Condition')
